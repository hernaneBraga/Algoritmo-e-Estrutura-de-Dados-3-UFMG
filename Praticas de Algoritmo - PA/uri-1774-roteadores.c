...
// Descrição do problema: https://www.urionlinejudge.com.br/judge/pt/problems/view/1774
...
void printArr(int arr[], int n, struct Graph* graph)
{
    int i;
    for (i = 1; i < n; ++i)
        printf("%d - %d ---- %d\n", arr[i]+1, i+1, graph->array[arr[i]].head->weight);
}
...
int main()
{
    ...
    int i, num_vertices, num_arestas, lixo;
    int aux1, aux2, aux3;

    
    lixo = scanf("%d %d", &num_vertices, &num_arestas);
    struct Graph* graph = createGraph(num_vertices);

   // printf("%d %d\n", num_vertices, num_arestas);
    for( i = 0 ; i < num_arestas ; i++){

        lixo = scanf("%d %d %d", &aux1, &aux2, &aux3);
       // printf("%d %d %d\n", aux1, aux2, aux3);
        addEdge(graph, aux1 - 1, aux2 - 1, aux3);
        lixo++;
    }
